...
type DeepFind<T, U extends string> = U extends keyof T
  ? { [K in U]: T[K] }
  : U extends `${infer First}.${infer Rest}`
    ? First extends keyof T
      ? { [K in First]: DeepFind<T[K], Rest>}
      : unknown
    : unknown

type UnionToIntersection<T> = (T extends unknown ? (x: T) => void : never) extends (y: infer P) => void ? P : never

type DeepPick<T, U extends string> = UnionToIntersection<DeepFind<T, U>>
